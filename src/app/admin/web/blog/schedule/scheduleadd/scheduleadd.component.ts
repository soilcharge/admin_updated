...
declare var $: any;
@Component({
  selector: 'app-scheduleadd',
  templateUrl: './scheduleadd.component.html',
  styleUrls: ['./scheduleadd.component.css']
})
export class ScheduleaddComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formContent = new FormGroup({
      ...
      photo_two: new FormControl('', [Validators.required]),
    ...
    if (this.editdata) {
      ...
      this.WebService.webBlogScheduleForEdit(obj).subscribe(res => {
        if (res['result'] == true) {
        ...
        if (res['error'] == true) {
          ...
          this.router.navigate(['/admin', 'blogschedule-list']);
        }
      });
  ...
  onSubmit() {
    ...
    if (this.formContent.value.datafor == 0) {
      if (!this.fileupone && !this.fileuptwo) {
        this.toastr.warning('Choose blog schedule photos!');
        return;
      }
      this.ngxService.start();
      this.WebService.webBlogScheduleAdd(this.formContent.value, this.fileupone, this.fileuptwo).subscribe(res => {
        if (res['result'] == true) {
          this.toastr.success("Blog schedule added successfully!");
          this.router.navigate(['/admin', 'blogschedule-list']);
        }
        else {
    ...
    if (this.formContent.value.datafor == 1) {
      this.ngxService.start();
      this.WebService.webBlogScheduleUpdate(this.formContent.value, this.fileupone, this.fileuptwo).subscribe(res => {
        if (res['result'] == true) {
          this.toastr.success("Blog schedule updated successfully!");
          this.router.navigate(['/admin', 'blogschedule-list']);
        }
        else {
          this.toastr.error(res['message']);
