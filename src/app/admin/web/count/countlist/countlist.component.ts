...
export class CountlistComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.webService.WebFrontCounterList().subscribe(datalist => {
  ...
  //       this.router.navigate(['/admin', 'redirectself'], { state: ['/admin', 'webcount-list'] });
  //     }
  //     if (res['error'] == true) {
  ...
  getForEdit(event) {
    this.editid = event;
    this.router.navigate(['/admin', 'webcount-edit', this.editid]);
  }
}
